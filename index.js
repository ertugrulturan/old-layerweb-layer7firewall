...
const blacklist_file = '/etc/nginx/lwblacklist.txt';
...
function BanIP(ip) {
	...
	if (already_banned == true) {
		...
		setTimeout(function () {
			...
			console.log("malicious traffic => " + ip);
...
function AddIpToVerif(ip, path) {
	...
	tableau_ip[ip].path.forEach(function (item) {
		if (path == item) {
			if (tableau_ip[ip].count > 15) {
				is_ok = true;
			}
		}
	});

	if (tableau_ip[ip].count > 70 && is_ok == true) {
		BanIP(ip);
	}

	if (tableau_ip[ip].count > 150) {
		BanIP(ip);
	}
